...
export class GroupsService {
  constructor(
    ...
    private expensesRepository: Model<ExpenseModel, ExpenseKey>,
  ) {}
  ...
  async addExpence(addExpenceDto: AddGroupExpenseDto) {
    ...
    return {
      status: true,
      message: 'Group Expense added',
    };
  }

  async addExpenceToUser(addExpenceDto: AddfriendExpenseDto) {
    const sender = addExpenceDto.sender;
  ...
  async getExpenceRelation(expenseDto: UserriendHistory) {
    const user1 = expenseDto?.user1;
    const user2 = expenseDto?.user2;
    let user1ToUser2 = 0;
    let user2ToUser1 = 0;
    let user1Expenses = await this.expensesRepository.find({
      sender: user1,
      reciever: user2,
    });
    if (user1Expenses) {
    ...
    let user2Expenses = await this.expensesRepository.find({
      sender: user2,
      reciever: user1,
    });
    if (user2Expenses) {
  ...
  async SettleUpExpense(expenseDto: UserriendHistory) {
    const user1 = expenseDto?.user1;
    const user2 = expenseDto?.user2;
    let user1ToUser2 = 0;
    let user2ToUser1 = 0;
    ...
    if (user1ToUser2 < user2ToUser1) {
      if (netAmmount > 0) {
        this.addExpenceToUser({
          sender: user1,
          reciever: user2,
          amount: netAmmount,
        });
        return {
          message: 'Settle Up Done',
          status: true,
        };
      } else {
        return {
          message: 'Already SettleUp is Done',
          status: true,
        };
      }
    } else if (user1ToUser2 == user2ToUser1) {
      return {
        message: 'Already SettleUp is Done',
        status: true,
      };
      ...
      return {
        message: 'User 2 Has record Payment with you based on Settlep.',
        status: true,
      };
    }
  }
